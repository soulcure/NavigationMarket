...
public class NewestActivity extends Activity implements OnScrollListener {
	...
	private void init() {
		...
		((TextView)findViewById(R.id.tvTitle)).setText(getResources().getString(R.string.title_newest));
		// 线性布局
		...
		String params = "m=api&c=rank&page=" + page++ + "&size="
				+ pageSize + "&ver="
				+ Util.getVersionName(NewestActivity.this);
		...
		recomendListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View convertView,
				...
				NewestActivity.this.startActivity(new Intent(
						NewestActivity.this, GameDetailActivity.class).putExtra("gameId", item.getGameId()));
			}
		});
	}
	...
	Handler mServiceHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case GET_ADV_XML_SUCCESS:
				advAdapter = new AdvAdapter(newestAdvList, NewestActivity.this);
				...
				for (int i = 0; i < newestAdvList.size(); i++) {
					ImageView pointView = new ImageView(NewestActivity.this);
