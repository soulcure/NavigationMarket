...
public class GameOfTypeActivity extends Activity implements OnScrollListener, OnClickListener {

	protected static final String TAG = "GameOfTypeActivity";
	...
	private List<AdvItem> newestAdvList = new ArrayList<AdvItem>(); // 精品推荐广告列表
	private List<GameItem> newestGameList = new ArrayList<GameItem>(); // 精品推荐数据
	...
	private void init() {
		...
		((TextView)findViewById(R.id.tvTitle)).setText(getIntent().getStringExtra("typeName"));
		// 线性布局
		...
		recommendListAdapter = new RecommendListAdapter(newestGameList, this);
		
		typeId = getIntent().getStringExtra("typeId");

		String params = "m=api&c=classify&classify=" + typeId + "&page=" + page++ + "&size="
				+ pageSize + "&ver="
				+ Util.getVersionName(GameOfTypeActivity.this);
		...
		recomendListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View convertView,
				...
				GameOfTypeActivity.this.startActivity(new Intent(
						GameOfTypeActivity.this, GameDetailActivity.class).putExtra("gameId", item.getGameId()));
			}
		});
	}
	...
	private void imgGalleryInit() {
		...
		String params = "m=api&c=classify_ad&classify=" + typeId;
		params += CenterUrlHelper.getSign(params, CenterUrlHelper.secret);
	...
	Handler mServiceHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case GET_RECOMMEND_GAME_XML_SUCCESS:
				recommendListAdapter.setmGameList(newestGameList);
			...
			case GET_ADV_XML_SUCCESS:
				advAdapter = new AdvAdapter(newestAdvList, GameOfTypeActivity.this);
				mRecommendAdvGallery.setAdapter(advAdapter);
				
				sumOfDrawble = newestAdvList.size();
				pointLinear.removeAllViews();
				for (int i = 0; i < newestAdvList.size(); i++) {
					ImageView pointView = new ImageView(GameOfTypeActivity.this);
